  ...
  <div class="space-y-6 pt-6">
    ...
    <div class="glass-card p-4 space-y-4">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <h3 class="text-lg font-semibold text-slate-900">{{ currentMonthDisplay }}</h3>
        <div class="flex items-center gap-2">
          <button type="button" class="btn-icon" aria-label="Previous month" @click="navMonth(-1)">‹</button>
          <button type="button" class="btn-text" @click="goToCurrentMonth">Current</button>
          <button type="button" class="btn-icon" aria-label="Next month" @click="navMonth(1)">›</button>
        </div>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
        <input v-model="localFilters.search" type="search" placeholder="Search description…" class="input-field" @input="debouncedFetch" />
        <select v-model="localFilters.category" class="input-field" @change="applyFilters">
        ...
        <select v-model="localFilters.sort" class="input-field" @change="applyFilters">
    ...
    <div class="glass-card p-6">
      <p class="text-sm text-slate-500 mb-4">{{ transactions.length }} transaction(s)</p>
      <div class="space-y-3">
        <div
        ...
        >
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex gap-2">
              <button class="text-xs text-primary-600 hover:underline" @click="openModal(transaction)">Edit</button>
              <button class="text-xs text-red-600 hover:underline" @click="remove(transaction.id)">Delete</button>
            </div>
          </div>
        </div>
        <div v-if="!transactions.length" class="text-center py-8 text-slate-500">No transactions found</div>
      </div>
    </div>

    <div v-if="showModal" class="fixed inset-0 bg-slate-900/40 z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 my-8">
        <h3 class="text-lg font-bold mb-4">{{ editingId ? 'Edit' : 'Add' }} Transaction</h3>
        <form class="space-y-4" @submit.prevent="save">
          <input v-model="form.description" class="input-field w-full" placeholder="Description" required />
          <input v-model.number="form.amount" type="number" step="0.01" class="input-field w-full" placeholder="Amount" required />
          <select v-model="form.type" class="input-field w-full">
            <option value="EXPENSE">Expense</option>
            <option value="INCOME">Income</option>
          </select>
          <input v-model="form.transactionDate" type="date" class="input-field w-full" required />
          <input v-model="form.category" class="input-field w-full" placeholder="Category" required />
          <div v-if="editingId" class="space-y-2 border-t pt-3">
            ...
            <button v-if="receiptFile" type="button" class="text-sm text-primary-600" @click="scanReceipt">Scan receipt (AI)</button>
            <a v-if="hasReceipt" :href="receiptUrl" target="_blank" class="text-sm text-primary-600 block">Download receipt</a>
          </div>
          <div class="flex gap-2">
...
const categories = [
  'FOOD_GROCERIES', 'FOOD_RESTAURANT', 'TRANSPORTATION', 'HOUSING_RENT', 'HOUSING_UTILITIES',
  'HOUSING_MAINTENANCE', 'HEALTHCARE', 'ENTERTAINMENT', 'SHOPPING', 'EDUCATION', 'TRAVEL',
  'INSURANCE', 'INVESTMENTS', 'SALARY', 'FREELANCE', 'OTHER'
]
...
const localFilters = ref({ search: '', category: '', type: '', sort: 'dateDesc' })
const form = ref({ description: '', amount: '', type: 'EXPENSE', category: '', transactionDate: new Date().toISOString().split('T')[0] })
...
const navMonth = (delta) => {
  ...
  budgetsStore.setCurrentMonth(transactionsStore.currentMonth, transactionsStore.currentYear)
  applyFilters()
}
...
const openModal = async (tx = null) => {
  ...
  if (tx) {
    ...
    } catch { hasReceipt.value = false }
  } else {
    form.value = { description: '', amount: '', type: 'EXPENSE', category: '', transactionDate: new Date().toISOString().split('T')[0] }
  }
...
const save = async () => {
  ...
  try {
    if (!form.value.category && form.value.description) {
      const cat = await resolveCategory(form.value.description, TRANSACTION_KEYWORDS)
      if (cat) form.value.category = cat
    }
    let tx
    if (editingId.value) {
      tx = await transactionsStore.updateTransaction(editingId.value, form.value)
...
onMounted(async () => {
  if (route.query.category) localFilters.value.category = String(route.query.category)
  transactionsStore.setFilters(localFilters.value)
