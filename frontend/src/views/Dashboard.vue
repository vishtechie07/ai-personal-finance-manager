  ...
  <div class="w-full mx-auto pt-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8">
      <div class="min-w-0">
        ...
        <p class="text-slate-600 mt-2">Welcome back, {{ username }}! Here's your financial overview.</p>
      </div>
      <div class="page-header-actions">
        <button type="button" @click="refreshDashboard" class="btn-secondary">
          <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
          </svg>
          Refresh
        </button>
        <button type="button" @click="showAddTransaction = true" class="btn-primary">
          <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
          </svg>
          Add Transaction
        </button>
        <button type="button" @click="showAddBudget = true" class="btn-secondary">
          <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    ...
    <div class="glass-card p-6 mb-8">
      <div class="flex items-center justify-between">
        <h3 class="text-lg font-semibold text-slate-900">Viewing Data for {{ currentMonthDisplay }}</h3>
    ...
    <div class="glass-card p-6 mb-6">
      <h3 class="text-lg font-semibold text-slate-900 mb-4">Quick add transaction</h3>
      <form class="grid grid-cols-1 md:grid-cols-5 gap-3 items-end" @submit.prevent="quickAdd">
        <input v-model="quickForm.description" class="input-field" placeholder="Description" required />
        <input v-model.number="quickForm.amount" type="number" step="0.01" class="input-field" placeholder="Amount" required />
        <select v-model="quickForm.type" class="input-field">
          <option value="EXPENSE">Expense</option>
          <option value="INCOME">Income</option>
        </select>
        <input v-model="quickForm.category" class="input-field" placeholder="Category" />
        <button type="submit" class="btn-primary" :disabled="quickSaving">{{ quickSaving ? 'Adding…' : 'Add' }}</button>
      </form>
    </div>
    ...
    <div v-if="dueSoonBills.length" class="glass-card p-6 mb-6">
      <div class="flex justify-between items-center mb-3">
        ...
        <router-link to="/bills" class="text-sm text-primary-600 hover:underline">View all</router-link>
      </div>
      <ul class="space-y-2">
        <li v-for="b in dueSoonBills" :key="b.id" class="flex justify-between text-sm">
    ...
    <div class="glass-card p-6 mb-6">
      ...
      <div v-if="!budgetHealthItems.length" class="text-slate-500 text-sm">No budgets for this month</div>
      <div v-else class="space-y-4">
        <div v-for="item in budgetHealthItems" :key="item.id">
          <div class="flex justify-between text-sm mb-1">
            <router-link :to="item.link" class="font-medium text-slate-900 hover:text-primary-600">{{ item.name }}</router-link>
            <span>{{ item.pct.toFixed(0) }}%</span>
          </div>
          <div class="h-2 bg-slate-200 rounded-full overflow-hidden">
            <div class="h-full rounded-full" :class="item.barClass" :style="{ width: Math.min(item.pct, 100) + '%' }" />
          </div>
        </div>
      </div>
    </div>

    <div v-if="attentionBudgets.length" class="glass-card p-6 mb-6 border border-amber-200 bg-amber-50/50">
      <h3 class="text-lg font-semibold text-amber-900 mb-3">Needs attention</h3>
      <ul class="space-y-2 text-sm text-amber-800">
        <li v-for="a in attentionBudgets" :key="a.id">
          <router-link :to="a.link" class="hover:underline">{{ a.message }}</router-link>
        </li>
      </ul>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
      <div class="glass-card p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <p class="text-sm font-medium text-slate-600">Current Month Balance</p>
            <p class="text-2xl font-bold text-slate-900">${{ currentMonthBalance.toFixed(2) }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="glass-card p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <p class="text-sm font-medium text-slate-600">Current Month Income</p>
      ...
      <div class="glass-card p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <p class="text-sm font-medium text-slate-600">Current Month Expenses</p>
      ...
      <div class="glass-card p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <p class="text-sm font-medium text-slate-600">Current Month Budgets</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4 w-full">
      <div class="glass-card p-6" style="height: 400px; overflow: hidden;">
        <h3 class="text-lg font-semibold text-slate-900">Spending by Category - {{ currentMonthDisplay }}</h3>
      ...
      <div class="glass-card p-6" style="height: 400px; overflow: hidden;">
        <h3 class="text-lg font-semibold text-slate-900">Monthly Overview - Last 6 Months</h3>
        <div style="height: 300px; position: relative; border: 1px dashed #ccc;">
    ...
    <div class="glass-card p-6 mb-4">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-semibold text-slate-900">Current Month Transactions</h3>
    ...
    <!-- Budget Alerts -->
    <div class="glass-card p-6">
        <h3 class="text-lg font-semibold text-slate-900 mb-4">Budget Alerts for {{ currentMonthDisplay }}</h3>
        <div class="space-y-3">
    ...
    <div v-if="showAddTransaction" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm overflow-y-auto h-full w-full z-50 flex items-center justify-center animate-fade-in">
      <div class="relative mx-auto p-8 border border-slate-200 shadow-xl rounded-2xl bg-white w-full max-w-md transform transition-all duration-300 animate-slide-up">
        <div class="mt-3">
          <h3 class="text-2xl font-bold text-slate-900 mb-6">Add New Transaction</h3>
            ...
            <div>
              ...
              <div class="relative">
                <input 
                  ...
                  :class="newTransaction.category && newTransaction.category !== 'Uncategorized' ? 'border-green-500 bg-green-50' : ''"
                >
    ...
    <!-- Add Budget Modal -->
    <div v-if="showAddBudget" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm overflow-y-auto h-full w-full z-50 flex items-center justify-center animate-fade-in">
      <div class="relative mx-auto p-8 border border-slate-200 shadow-xl rounded-2xl bg-white w-full max-w-md transform transition-all duration-300 animate-slide-up">
        <div class="mt-3">
          <h3 class="text-2xl font-bold text-slate-900 mb-6">Create New Budget</h3>
            ...
            <div>
              ...
              <div class="relative">
                <input 
                  ...
                  :class="[
                    'input-field pr-20',
                    isBudgetAnalyzing ? 'border-yellow-500 bg-yellow-50' : 
                    isBudgetSmartSuggested ? 'border-green-500 bg-green-50' : ''
                  ]"
...
export default {
  ...
  setup() {
    ...
    const quickForm = ref({ description: '', amount: '', type: 'EXPENSE', category: 'OTHER' })
    ...
    const budgetHealthItems = computed(() => {
      return (budgetsStore.budgetsByMonth || []).map((b) => {
        const pct = (parseFloat(b.spentAmount || 0) / parseFloat(b.amount || 1)) * 100
        let barClass = 'bg-green-500'
        if (pct >= 90 || parseFloat(b.remainingAmount || 0) < 0) barClass = 'bg-red-500'
    ...
    const quickAdd = async () => {
      ...
      try {
        ...
        quickForm.value = { description: '', amount: '', type: 'EXPENSE', category: 'OTHER' }
